...
#     user_norm_rank.write.parquet(f'hdfs:/user/yz4315_nyu_edu/user_norm_rank.parquet')
...
#     train.write.parquet(f'hdfs:/user/yz4315_nyu_edu/ALS_train.parquet')
#     print("ALS_train.parquet complete")
    
#     validation.write.parquet(f'hdfs:/user/yz4315_nyu_edu/ALS_validation.parquet')
...
#     print('begain read')
    train = spark.read.parquet(f'hdfs:/user/yz4315_nyu_edu/ALS_train.parquet')
    validation = spark.read.parquet(f'hdfs:/user/yz4315_nyu_edu/ALS_validation.parquet')
    ranking = spark.read.parquet(f'hdfs:/user/yz4315_nyu_edu/user_norm_rank.parquet')
    ...
    for reg in regs:
        for r in rs:
            for a in alphas:
                ...
                ALSmodels.append(als)
                prameter_sets.append([reg, r, a])
                
    print('begin tune')
    for idx, als in enumerate(ALSmodels):
        print('total:', len(ALSmodels), 'currently at:', idx)
        model = als.fit(train)
...
#         trackSubSetRecs = model.recommendForItemSubset(tracks, 100)
        ...
        if ndcg_at_k > best_ndcg:
            best_ndcg = ndcg_at_k
            best_parameter_sets = prameter_sets[idx]
        if mean_ap > best_meanAP:
            best_meanAP = mean_ap
            best_parameter_AP = prameter_sets[idx]
        print('current best parameter sets:', best_parameter_sets, 'with best ndcg@100:', best_ndcg)
        print('current best parameter sets:', best_parameter_AP, 'with best meanAP@100:', best_meanAP)
            
    print('Parameter setting with best performance:', best_parameter_sets, 'with ndcg@100:', best_ndcg)
    print('Parameter setting with best performance:', best_parameter_AP, 'with meanAP@100:', best_meanAP)
...
#     #validation_interaction.write.parquet(f'hdfs:/user/yz4315_nyu_edu/interactions_validation_small.parquet')
#     #train_interaction = spark.read.csv('data_test.csv', schema='user_id INT, recording_msid STRING')
#     train = spark.read.parquet(f'hdfs:/user/yz4315_nyu_edu/train_small_norm.parquet')
#     validation = spark.read.parquet(f'hdfs:/user/yz4315_nyu_edu/vali_small_norm.parquet')
...
# # #     train.show()
# # #     validation.show()
# #     train.write.parquet(f'hdfs:/user/yz4315_nyu_edu/train_small_norm.parquet')
# #     validation.write.parquet(f'hdfs:/user/yz4315_nyu_edu/vali_small_norm.parquet')
#     train.createOrReplaceTempView('train')
#     validation.createOrReplaceTempView('validation')
#     train = spark.sql('SELECT * FROM train WHERE user_id IS NOT NULL and track_new_id is not NULL and normalized_ranking IS NOT NULL')
#     validation = spark.sql('SELECT * FROM validation WHERE user_id IS NOT NULL and track_new_id is not NULL and normalized_ranking IS NOT NULL')
#     train = train.withColumn("user_id",col("user_id").cast(IntegerType())) \
#         .withColumn("track_new_id",col("track_new_id").cast(IntegerType())) \
#         .withColumn("normalized_ranking",col("normalized_ranking").cast(DoubleType()))
#     validation = validation.withColumn("user_id",col("user_id").cast(IntegerType())) \
#         .withColumn("track_new_id",col("track_new_id").cast(IntegerType())) \
#         .withColumn("normalized_ranking",col("normalized_ranking").cast(DoubleType()))
#     train = train.drop(train.ranking)
#     validation = validation.drop(validation.ranking)
...
#     print('begin convert the df to rdd')
#     # Convert the DataFrame to an RDD
#     predictions_and_labels_rdd = predictions_and_labels.rdd.map(tuple)
